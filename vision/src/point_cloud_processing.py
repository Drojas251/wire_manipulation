...
def callback(data):
    ...
    # Find outliers 
    outliers = []
    threshold = 0.12 # a point whos closest neighbor is greater than 0.1m away is considered an outlier 
    ...
    if distance_between_y_extrema > 0.1:
        print("Y end point")
        print("end point is either" + str(extrema_elements[0]) + "or" + str(extrema_elements[1]))
        end_point = int(extrema_elements[0])
    else:
        print("Z end point")
        print("end point is either" + str(extrema_elements[2]) + "or" + str(extrema_elements[3]))
    ...
    """for j in range(len(sorted_points)):
        ...
        marker_object.header.stamp = rospy.get_rostime()
        marker_object.ns = 'point'
        marker_object.id = count + j
        marker_object.type = Marker.SPHERE
        marker_object.action = Marker.ADD

        my_point = Point()
        my_point.x = sorted_points[j,0]
        my_point.y = sorted_points[j,1]
        my_point.z = sorted_points[j,2]
